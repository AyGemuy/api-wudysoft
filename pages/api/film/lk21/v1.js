...
class LK21 {
  ...
  async req({ url, method = "GET", params = {}, data = {}, headers = {}, ...rest }) {
    try {
  ...
  /* ---------- parser item poster (home / listing / related) ---------- */
  parseItem($, el) {
  ...
  /* ---------- HOME ---------- */
  async home({ ...rest } = {}) {
    const $ = await this.req({ url: "/", ...rest });
    if (!$) return null;

    const sections = $(".widget[data-type]")
      .map((_, w) => {
        const id = $(w).attr("id");
        ...
        const items = $(w)
          .find("li.slider, #you-may-also-like li")
          .map((_, el) => this.parseItem($, el))
          .get();
    ...
    const latest = $("#post-container article, .gallery-grid article")
      .map((_, el) => this.parseItem($, el))
      .get();
  ...
  /* ---------- BROWSE ---------- */
  async browse({ path = "/populer", page = 1, type = "", ...rest } = {}) {
    ...
    const items = $(".gallery-grid article, #post-container article")
      .map((_, el) => this.parseItem($, el))
      .get();
    ...
    return { title, page: parseInt(page), totalPages: last, items };
  }

  /* ---------- GENRES, COUNTRIES, YEARS ---------- */
  async getGenres({ ...rest } = {}) {
    const $ = await this.req({ url: "/genre/", ...rest });
    if (!$) return [];

    const raw = $('a[href^="/genre/"]')
      .map((_, a) => ({
  ...
  async getCountries({ ...rest } = {}) {
    const $ = await this.req({ url: "/country/", ...rest });
    if (!$) return [];
    ...
    $('a[href^="/country/"]').map((_, a) => {
      ...
      if (s) {
        map.set(s, { slug: s, name: $(a).text().trim() });
      }
    });
    return [...map.values()];
  }
  ...
  async getYears({ ...rest } = {}) {
    const $ = await this.req({ url: "/year/", ...rest });
    if (!$) return [];

    const raw = $('a[href^="/year/"]')
      .map((_, a) => $(a).attr("href").split("/").filter(Boolean)[1])
      .get();
  ...
  /* ---------- SEARCH ---------- */
  async search({ query = "", page = 1, ...rest } = {}) {
    ...
    return { query, page: parseInt(page), totalPages: data.totalPages || data.total_pages || 1, items };
  }
  ...
  async detail({ url = "", ...rest } = {}) {
    ...
    if (openNow) {
      return this.detailSeries({ url: openNow, ...rest });
    }
    return this.detailMovie({ $, url, ...rest });
  }

  parseDetailCore($) {
    const infoTag = $(".info-tag span")
      .map((_, el) => $(el).text().trim())
      .get();

    const tags = $(".tag-list .tag a")
      .map((_, el) => ({
  ...
  async detailMovie({ $, url, ...rest } = {}) {
    if (!$) {
      $ = await this.req({ url, ...rest });
      if (!$) return null;
    }
    const core = this.parseDetailCore($);

    const playLinks = $(".movie-action a")
      .map((_, a) => {
    ...
    const players = $("#player-list a[data-url], #player-list li a")
      .map((_, a) => {
  ...
  async detailSeries({ url, ...rest } = {}) {
    ...
    const related = $(".widget[data-type] li.slider")
      .map((_, el) => this.parseItem($, el))
      .get();
  ...
  async stream({ url = "", ...rest } = {}) {
    ...
    const players = $("#player-list a[data-url], #player-list li a")
      .map((_, a) => {
        ...
        const u = $(a).attr("data-url") || $(a).attr("href");
        if (u && u !== "#") {
          return { server, url: u, active: $(a).hasClass("active") };
        }
        return null;
      })
      .get()
    ...
    const nav = $('a:contains("EPISODE SEBELUMNYA"), a:contains("EPISODE BERIKUTNYA"), .prev-episode a, .next-episode a')
      .map((_, a) => {
        const href = $(a).attr("href');
        if (href && /^\/|^https?:/.test(href)) {
          return { text: $(a).text().trim(), url: href };
        }
        return null;
      })
      .get()
