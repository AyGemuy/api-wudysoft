...
class NanoBanana {
  ...
  async i2i({ prompt = '', image, num = 1, model = 'nano-banana', fmt = 'png', ar = '1:1', device_id = '1', is_paid = 'true', ...rest }) {
    try {
  ...
  async buf(src) {
    try {
      ...
      if (src.startsWith('http://') || src.startsWith('https://')) {
        const res = await axios.get(src, { responseType: 'arraybuffer', headers: {
