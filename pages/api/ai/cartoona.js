...
class Cartoona {
  ...
  log(lvl, msg, ...a) {
    const icon = { 'INFO': '🔹', 'SUCCESS': '✅', 'WARN': '⚠️', 'ERROR': '❌' }[lvl] || '▪️';
  ...
  async top_up(target = 200) {
    ...
    while (this.bal < target) {
      ...
      try {
        const r = await this.http.post('/functions/v1/reward-credits', { device_id: this.id });
  ...
  /**
   * Mengambil daftar style yang tersedia
   */
  async styles({ lang = 'id', select = '*', active = true, ...rest }) {
