...
class ResikuBypass {
  constructor() {
   ...
   expedisiList() {
    ...
    try {
      const list = Object.entries(this.map).map(([code, name]) => ({ code, name }));
      ...
      return { success: false, error: err?.message || 'Failed to load expedisi list', data: [] };
    }
  }

  async trackResi({ resi, expedisi, ...rest }) {
    console.log(`Tracking resi: ${resi}, expedisi: ${expedisi}`);
    try {
      if (!resi)     return { success: false, error: 'Nomor resi wajib diisi!', resi, expedisi };
      if (!expedisi) return { success: false, error: 'Kurir/Ekspedisi wajib diisi!', resi, expedisi };

      const cleanWb = resi.replace(/\s+/g, '').trim();
      const code    = expedisi.toLowerCase();

      if (cleanWb.length < 5) return { success: false, error: 'Nomor resi terlalu pendek!', resi, expedisi };
      if (!this.map[code])    return { success: false, error: `Ekspedisi "${expedisi}" belum didukung!`, resi, expedisi };

      // FIS Token
      console.log('Mengautentikasi Firebase Installation (FIS)...');
      const resFis = await axios.post(
        `${FIS_URL}/v1/projects/${PROJECT}/installations`,
        { fid: 'cVDL7Rj2TpWUfIb43r7OLN', appId: this.appId, authVersion: 'FIS_v2', sdkVersion: 'a:19.0.1' },
        { headers: { ...this.hds, 'x-goog-api-key': this.key } }
      );
      ...
      const resApp = await axios.post(
        `${FAC_URL}/v1/projects/${PROJECT}/apps/${this.appId}:exchangeDebugToken`,
        { debugToken: this.dbg },
        { params: { key: this.key }, headers: { ...this.hds, 'Authorization': `Firebase ${fisToken}` } }
      );
      ...
      const resTrack = await axios.post(
        `${this.api}/tracking`,
        { wb_number: cleanWb, courier: code, uuid: '3c0a4e8d-88f1-4ab4-9fa2-8cb14ef82121', ...rest },
        { headers: { ...this.hds, 'X-Firebase-AppCheck': appCheckToken } }
      );

      const raw = resTrack.data;
      if (!raw || raw.status !== 1) {
        console.warn('Server merespon namun data tidak ditemukan.');
        return { success: false, error: raw?.message || 'Data resi tidak ditemukan.', resi, expedisi };
      }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    switch (action) {
      case 'check':
        if (!params.resi) {
          return res.status(400).json({ error: 'Silakan masukkan nomor resi.' });
        }
        if (!params.expedisi) {
