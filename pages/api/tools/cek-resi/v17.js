...
class CekResi {
  ...
  _parseDate(dateStr) {
    ...
    // support bulan Inggris (dari response asli) dan Indonesia
    const months = {
  ...
  async expedisiList() {
    ...
    try {
      const { data } = await this.http.get(this.listURL);
      if (data?.status !== "berhasil" || !Array.isArray(data.data)) {
        return { success: false, code: 500, error: "Gagal mengambil daftar kurir" };
      }
      const couriers = data.data
        .map(c => {
          const name = c.title?.trim();
          if (!name) return null;
          return { name, format: this._toFormat(name) };
        })
        .filter(Boolean);
      ...
      const result = { success: true, code: 200, couriers };
      this.cache = { ...result, _formatToName: formatToName, _nameToFormat: nameToFormat };
      return result;
    } catch (err) {
      return { success: false, code: err.response?.status || 500, error: "Jaringan error" };
    }
  }

  async trackResi({ resi, expedisi, ...rest }) {
    if (!resi?.trim() || !expedisi?.trim()) {
      return { success: false, code: 400, error: "resi dan expedisi wajib diisi" };
    }
    ...
    const { _formatToName, _nameToFormat } = this.cache;
    ...
    const payload = qs.stringify({ resi: resi.trim(), ex: courierName, ...rest });
    try {
      const { data } = await this.http.post(this.trackURL, payload, {
