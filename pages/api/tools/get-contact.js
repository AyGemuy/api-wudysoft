...
class GetContact {
  constructor(config = {}) {
    this.base = config.base || "https://tools.naufalist.com";
    this.solvUrl = config.solvUrl || "https://cf.kanaa.eu.cc/solve";
    this.siteKey = config.siteKey || "0x4AAAAAABut31SL98evGtHx";
  ...
  async _sol(pageUrl, maxRetries = 3, retryDelay = 3e3) {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.log(`[Process] Menyelesaikan bypass Turnstile (Percobaan ${attempt}/${maxRetries})...`);
        const res = await axios.post(this.solvUrl, {
          url: pageUrl || `${this.base}/getcontact`,
        ...
        if (token) {
          console.log("[Process] Turnstile berhasil diselesaikan");
          return {
            status: true,
            result: token
          };
        ...
        console.warn(`[Warning] Percobaan bypass Turnstile ke-${attempt} menemui kendala: ${err?.message || err}`);
        if (attempt < maxRetries) {
          console.log(`[Process] Menunggu ${retryDelay / 1e3} detik sebelum mencoba kembali...`);
          await new Promise(resolve => setTimeout(resolve, retryDelay));
        } else {
          return {
            status: false,
            error: `Server penyedia bypass Turnstile (${this.solvUrl}) tidak merespon dengan benar (Error: ${err?.message || err}). Silakan periksa kembali kestabilan server pemecah captchas atau coba lagi beberapa saat.`
          };
        }
      }
    }
  ...
  async search({
    ...
    if (!turnstileToken) {
      const solRes = await this._sol(`${this.base}/getcontact`);
